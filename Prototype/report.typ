...
#table(
  ...
  [*Thành phần*], [*Vai trò*],
  [`Prototype`],
  [Interface hoặc abstract class khai báo phương thức `Clone()`. Nó định nghĩa contract chung cho các object có khả năng tự sao chép.],

  [`ConcretePrototype`],
  [Class cụ thể có state và cài đặt logic clone. Mỗi class tự quyết định clone shallow, deep hay clone có chọn lọc.],

  [`Client`], [Đối tượng sử dụng prototype để tạo object mới. Client không cần biết chi tiết constructor phức tạp.],
  [`Prototype Registry`],
  [Kho lưu các prototype theo key. Thành phần này không bắt buộc, nhưng rất hữu ích khi hệ thống có nhiều object mẫu cần quản lý.],
)
...
#table(
  ...
  [*Ưu điểm*], [*Giải thích*],
  [Giảm chi phí khởi tạo object phức tạp],
  [Object mẫu đã được cấu hình sẵn, client chỉ cần clone thay vì dựng lại từ đầu.],

  [Giảm code lặp], [Khi cần nhiều object gần giống nhau, ta không phải lặp lại toàn bộ constructor hoặc logic setup.],
  [Không phụ thuộc trực tiếp vào constructor cụ thể],
  [Client chỉ biết interface `Clone()`, không cần biết class có bao nhiêu constructor hoặc cần tham số gì.],

  [Dễ tạo biến thể object],
  [Clone object mẫu rồi chỉnh sửa một vài thuộc tính riêng như level, position, title hoặc theme.],
...
#table(
  ...
  [Clone object phức tạp không phải lúc nào cũng dễ],
  [Nếu object có nhiều quan hệ, object con, tài nguyên ngoài hoặc vòng tham chiếu, clone sẽ khó cài đặt đúng.],

  [Dễ nhầm giữa shallow copy và deep copy],
  [Nếu copy reference sai cách, object clone và object gốc có thể ảnh hưởng lẫn nhau.],

  [Có thể che giấu logic tạo object],
  [Client chỉ thấy clone, nên đôi khi khó biết object mới thật sự được tạo như thế nào.],

  [Không phù hợp nếu object đơn giản],
  [Nếu object chỉ có vài field và constructor đơn giản, Prototype có thể làm thiết kế rườm rà hơn.],

  [Cần chú ý identity và resource],
  [Các field như ID, connection, file handle, event subscription không nên bị clone máy móc.],
)
