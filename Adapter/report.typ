...
#table(
  ...
  [Mục đích chính],
  [Chuyển đổi interface không tương thích thành interface client mong muốn.],
  [Đơn giản hóa cách sử dụng một hệ thống con phức tạp.],

  [Vấn đề giải quyết],
  [Client không dùng được class có sẵn vì khác interface.],
  [Client không muốn làm việc trực tiếp với nhiều class hoặc nhiều bước phức tạp.],

  [Có thay đổi interface không?],
  [Có. Adapter biến interface của `Adaptee` thành `Target`.],
  [Có thể có, nhưng mục tiêu chính là gom và đơn giản hóa thao tác.],

  [Thường bọc], [Một class hoặc một service không tương thích.], [Nhiều class hoặc cả một subsystem.],
  [Ví dụ],
  [`StripePaymentAdapter` biến `StripeService` thành `IPaymentGateway`.],
  [`OrderFacade` gọi Inventory, Payment, Shipping để hoàn tất đơn hàng.],
)
...
#table(
  ...
  [Mục đích chính],
  [Làm interface không tương thích trở nên tương thích.],
  [Thêm chức năng mới cho object mà không sửa object gốc.],

  [Interface sau khi bọc], [Thường khác với interface của `Adaptee`.], [Thường giống interface của object gốc.],
  [Có thay đổi hành vi không?],
  [Có thể có chuyển đổi lời gọi và dữ liệu.],
  [Có, bằng cách bổ sung hành vi trước/sau khi gọi object gốc.],

  [Tập trung vào], [Tương thích interface.], [Mở rộng chức năng.],
  [Ví dụ],
  [`StripePaymentAdapter` biến `StripeService` thành `IPaymentGateway`.],
  [`LoggingPaymentGateway` thêm logging trước/sau khi thanh toán.],
)
...
#table(
  ...
  [Tách abstraction khỏi implementation để cả hai phát triển độc lập.],

  [Thời điểm dùng],
  [Thường dùng sau khi đã có class hoặc service không tương thích.],
  [Thường được thiết kế từ đầu để hệ thống dễ mở rộng.],

  [Quan hệ chính],
  [`Adapter` bọc hoặc giữ tham chiếu đến `Adaptee`.],
  [`Abstraction` giữ tham chiếu đến `Implementor`.],

  [Vấn đề chính],
  [“Tôi có class này nhưng interface không khớp.”],
  [“Tôi muốn abstraction và implementation thay đổi độc lập.”],
)
...
#table(
  ...
  [Vai trò],
  [Làm cho object không tương thích dùng được.],
  [Thêm lớp đại diện để lazy loading, caching, authorization, remote access.],
